...
DOGMA = [
    ...
    "일반인 사례입니다",
    # 전문 트레이너 정형구 (H20)
    # 사장님이 4/4 발행본에 모두 넣으신 문장이다. corpus 4편에도 있다.
    # CTA와 같은 성격의 브랜드 정형구이므로 반복이 정상이다
    "호흡과 발성은 생각보다 단순한 방법으로 해결되지 않습니다",
    "17년 경력 전문 트레이너가",
    "전문 트레이너가 1:1로",
    "보이스 닥터는 전문 트레이너가",
    "여러분 상태를 확인하며 진행합니다",
...
def normalize(text):
    ...
    도그마 문구를 걷어내기 전에 **공백을 먼저 한 칸으로 정규화한다.**
    wrap_lines.py 가 문구 한가운데에 줄바꿈을 넣으면 리터럴 비교가
    깨지기 때문이다. (실제로 이 순서 때문에 CTA 정형구가 오탐으로 잡혔다)
    """
